...
export class SchedulerComponent implements OnInit {
  ...
  async getAvailableTimes(date: string, companyId: number) {
    ...
    if (date && this.dateReg.test(date) && companyId) {
      this.schedulerService
        .listSchedules(date, companyId, true)
        .subscribe((data) => {
          this.times = data;
        });
    }
  ...
  checkIfAllTimesForTodayAreUnavailable(): boolean {
    return (
      ...
      this.companyId != null &&
      (this.times == null || this.times.length === 0)
    );
  }
  ...
  async submitForm(clientScheduleFormValue: any) {
    ...
    if (this.clientScheduleForm.valid) {
      ...
      await this.schedulerService
        .postReservedSchedule(clientScheduler)
        .subscribe(
          () => {
            this.submitResultService.setResultSubmitResultText(
              "Agendamento realizado com sucesso!",
              "Essa sessão foi reservada para você, até já!"
            );
          ...
          () => {
            this.submitResultService.setResultSubmitResultText(
              "Erro",
              "Não foi possível realizar seu agendamento. Favor nos avisar em contato@blindbeez.com.br"
            );
            this.submitResultService.setResultOkButton(
